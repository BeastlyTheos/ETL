...
public class Simulation
{
    ...
    public static void Main()
    {
        ...
        double time;
        LinkedListPriorityQueue<Event> events = new LinkedListPriorityQueue<Event>();
        Event e;
        CarArrivalEvent ce;
            ...
            do
          {
                ...
                for (int i = 0; directions.Length > i; i++)
                {
                    ...
                    directions[i].LightDuration = inputDouble;
                                        directions[i].Reset();
                    events.Add(new CarArrivalEvent(EventType.CarArriving, RandomExponential(directions[i].MeanArrivalTime), i));
                }//end of getting and resetting  light durations

        //add first light change
                events.Add(new Event(EventType.LightChange, time + directions[0].LightDuration));

//run simulation for 100 lightcycles
                while (MAX_LIGHTCYCLES > numLightCycles)
                {                    
                   e = events.Front(); events.Remove(); //pop and remove first top event
                                        time = e.Time; //increment time to current event
...
Console.Write("greenlight#{1} for {0}> ", directions[greenLight].Name, numLightCycles);
                ...
                for (int i = 0; directions.Length > i; i++)
                    while (0 < directions[i].numWaiting())
